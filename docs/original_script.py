...
# Constants to be used in calculation of plasma properties.

e = 1.602*10**-19                  # Elementary charge in C
epsilon_0 = 8.8541878188*10**-12   # Permittivity of free space in F/m
k = 1.38*10**-23                   # Boltzmann constant in J/K
M_Ar = 6.62*10**-26                # Mass of argon ion in kg
m = 9.11*10**-31                   # Mass of electron
J_eV = 6.241509*10**18             # Conversion factor for joules to eV
L = 33.4*10**-3                   # Length of langmuir probe         
d = 1.27*10**-3                    # Diameter of langmuir probe
m_Xe = 2.1801714*10**-25           # Mass of xenon in kg 
Ap = math.pi/4*d**2+math.pi*d*L    # Area of LP in m^2

# Area from LP probe for Pace's validation data set
# A =   0.738*10**-4

# Loading directory where LP data is stored

Files = glob.glob('C:/Users/nnuzz_rpjwflj/OneDrive/Documents/Asif-Nick Business & Thruster Stuff/2025 LP Data Analysis/52925/500th Recycle/*')


# Load Files. Creates a pandas dataframe that can be accessed by later functions to identify where the current values are in the files
DF = BatchRd(Files)

# Removing files that have bad data
BadDatDel(Files,DF)

# Create new array for dataframe with removed bad data. This also organizes the files in the same order as they appear in the directory
# with the "sorted() function so the returned zero cross values are organized in the same order the files appear in the directory.

F = sorted(glob.glob('C:/Users/nnuzz_rpjwflj/OneDrive/Documents/Asif-Nick Business & Thruster Stuff/2025 LP Data Analysis/52925/500th Recycle/*'))  # Reloading the directory without the bad data if any was found
print(f'File is {F[0]}')
# Initializing current & voltage values

I = VC(F)[0]
V = VC(F)[1]

# Perform zero cross on all files
I_ZeroX = ZeroCross(F,I)

#print(I_ZeroX)
#for i in range(len(I_ZeroX)):
#    print(I[I_ZeroX[i]])
    
#I_ZeroX = I_ZeroX[~np.iscomplex(I_ZeroX)]
#I_ZeroX = np.real(I_ZeroX)

#for i in range(len(I_ZeroX)):
#    print(I[I_ZeroX])
    #if isinstance(I_ZeroX, complex):
    #    os.remove(F[i])

# Ranges of values for current and their corresponding voltage values around the region of data crossing the x-axis (voltage)
I_ranges = np.array(ZCAnalysisRng(I_ZeroX,I,V)[0])
V_ranges = np.array(ZCAnalysisRng(I_ZeroX,I,V)[1])


# Calculating floating potential for each data file
Vfloat = FloatingPot(I_ranges,V_ranges)

ToRemove = []           # Indices to remove

for i in range(len(Vfloat)):
    
    # Here we check if the value for Vfloat is a floating point data type. If not, it is removed from the folder
    if isinstance(Vfloat[i], np.floating) == False:
        try:
            shutil.copy2(F[i], os.path.dirname(os.path.dirname(F[0])+'/Bad Data'))
...
print(f'The following file indices are being removed due to having complex solutions to the floating potential: {ToRemove}')
print(F)
if len(ToRemove) != 0:
    # Resizing all relevant arrays for calculation after removal of bad data sets with the complex solutions to floating potential
    Vfloat = np.delete(Vfloat,ToRemove)
    
    # If all Vfloat values were converted to imaginaries, this converts the array back to floating point data types
    Vfloat = Vfloat.astype(np.float64)
    
    I = [np.delete(I[idx],ToRemove) for idx, val in enumerate(I)] 
    V = [np.delete(V[idx],ToRemove) for idx, val in enumerate(V)]
    I_ZeroX = np.delete(I_ZeroX,ToRemove) 

# After resizing the F and Vfloat arrays the I and V arrays need to be resized as well. See above ^

print(f'Floating potentials are {Vfloat}')

# PlotData(F,I_ZeroX,V_ranges,I_ranges,VPlasma_Num,Vfloat)

# Calculating the electron current

Ecurrent = ECurr(Vfloat,F,I,V)
EC = Ecurrent[0]

# Calculating plasma potential  first attempting to use the second derivative method

#VPL = PlasmaPot(F,I_ZeroX,Ecurrent[0],V)
VPL = PlasmaPot(F,I_ZeroX,Ecurrent[0],V)

print(f'Plasma potentials are {VPL}')

# Calculating the electron temperatures
fig1 = plt.figure()
ax1 = fig1.add_subplot(111)
ET_i = ElectronTemp(F,Ecurrent[0],Vfloat,VPL,V,M_Ar,m)
ET = ET_i[0]
R = ET_i[1]

# The below will remove files that have negative electron temperatures
# This might be able to be remedied by changing the process for calculating the range over which to take the linear regression used to calculate the 
# electron temperature. This can be an update in the next version.
...
F = sorted(glob.glob('C:/Users/nnuzz_rpjwflj/OneDrive/Documents/Asif-Nick Business & Thruster Stuff/2025 LP Data Analysis/52925/500th Recycle/*'))
print(f'toremove {len(ToRemove)}')  

# Resizing all relevant arrays for calculation after removal of bad data sets with the negative electron temperatures
if len(ToRemove) != 0:   
    ET = np.delete(ET,ToRemove)
    I = [np.delete(I[idx],ToRemove) for idx, val in enumerate(I)] 
    V = [np.delete(V[idx],ToRemove) for idx, val in enumerate(V)]
    I_ZeroX = np.delete(I_ZeroX,ToRemove) 
    Vfloat = np.delete(Vfloat,ToRemove)
    VPL = np.delete(VPL,ToRemove)
    EC = np.delete(EC,ToRemove)

print(f'electron currents are {EC[0]}')
print(f'ELectron Temps are {ET}')
print(f'Correlation coefficients for electron temperature slope method are {R}')
# Calculating all remaining plasma properties

PProps = PlasmaProps('cylindrical',e,Ap,d/2,Vfloat,VPL,EC,M_Ar,m,ET,I_ZeroX,F,I,V)

'''
6/18/25 ISSUE: Tried running try/except on PProps below to resolve impossible solutions that are causing math domain errors due to negatives
in the square root terms of some of the plasma parameter calculations. It appears there are still files that have abnormal LP traces that
need to be removed. These are causing problems with the calculation of the plasma potential. For example some files demonstrate asymptotic 
behavior near the most negative values of probe biases and then show a linearly increasing curve with no indication of a transition region.
They also appear to have abnormally low current values. On the order of 10^-9 and 10^-10 amps. These files have not been identified by the 
script's existing methods for capturing bad data and removing it so some new methods will need to be employed. Files with asymptotic behavior
and/or abnormally low current values need to be removed. It may be possible to purge all these abnormal files simply by looking at all current values
in the file and making sure they are not all below a certain threshold indicating abnormal data. Since this is a problem with the raw data itself, this
can probably be remedied by modifying the existing BadDatDel function
'''

'6/22/25 UPDATE: The extra bad data files have been successfully identified and removed from the LP data sets'

'The following code does not work, but in future revisions it will be used to switch to Lobbias method if the second derivative method fails'
'''
global NE_i 

try:
    PProps = PlasmaProps('cylindrical',e,Ap,d/2,Vfloat,VPL,Ecurrent[0],M_Ar,m,ET_i[0],I_ZeroX,F,I,V)
    NE_i = PProps[0]
    
except:
    # Checking if there are any negative electron number density values. If so, the script will run Lobbia's method since this means the second derivative method found the plasma potential to be lower than the floating potential which is not possible
    
    for i in range(len(NE_i)):
        if NE_i[i] < 0:
            VPL = PlasmaPot_v2(F[i],Ecurrent[i][0],V[i])
            ET_i = ElectronTemp(F[i],Ecurrent[i][0],Vfloat[i],VPL,V[i],M_Ar,m)
            PProps[i] = PlasmaProps('cylindrical',e,Ap,d/2,Vfloat[i],VPL,Ecurrent[i][0],M_Ar,m,ET_i[0],I_ZeroX[i],F[i],I[i],V[i])
'''   
# Initial values for plasma properties. If iteration is needed, these will be updated

NE_i = PProps[0]
NI_i = PProps[1]
I_Isat_i = PProps[2]
RpL_i = PProps[3]
print(f'ion saturation current is {I_Isat_i}')

fig2 = plt.figure()
ax2 = fig2.add_subplot(111)
PlotData(F,I_ZeroX,V_ranges,I_ranges,VPL,Vfloat)

# Below we are checking if any of the data sets resulted in non-thin sheath cases. If they did, then the solution needs to be iterated until convergence is achieved.
CFiles = []

for i in range(len(PProps[3])):
    if PProps[3][i] < 50:
        CFiles.append(i)

if not CFiles:
    print('No corrections necessary. Analysis complete')
    
else:
    print(f'Corrections need to be made for ion density calculation. Iterate until convergence of the ion saturation current on the following files: {CFiles}')
    
    
    # Continue with implementing correction in the following iterative method
    
    
    # Shape of LP
    Shape = 'cylindrical'
    
    # Creating instance of LP class
    S = LP(Shape)
    '''
    # Next we can apply the iterative method using the above functions in the same order to obtain the plasma properties for successive iterations. Because each function runs all files at the same time,
    # for each area value we will only use the values calculated that share the same index location of that area value. This is because each index represents one measurement (LP sweep). So each function will
    # calculate values for all files, but we will only take A1 with Te1 for instance. Te3 calculated for A1, for example, is meaningless for these iterations and will not be reported or used. 
    
    # Select desired iterations
    
    NIter = 10
    
    # Need to initialize arrays for the iteration. For this test we will do 20 iterations. So beginning we have inital values for each file. This is an array of size 5. We would like to iterate on each of these
    # values 20 times 
    RpL = np.zeros((len(F),NIter))
    NE = np.zeros((len(F),NIter))
    NI = np.zeros((len(F),NIter))
    I_Isat = np.zeros((len(F),NIter))
    VP = np.zeros((len(F),NIter))
    ET = np.zeros((len(F),NIter))
    As = np.zeros((len(F),NIter))
    CV = np.zeros((len(F),NIter))


    # Begin implementation of iterations
    ...
    for j in range(len(As)):
        ...
        while Iter < NIter:
            ...
            if Iter == 0:
                # Setting initial values. These will be used to calculate the values for iteration 1 and then so on. So all values calculated using iteration 0 are values for iteration 1.
                RpL[j][Iter] = RpL_i[j]
                NE[j][Iter] = NE_i[j]
                NI[j][Iter] = NI_i[j]
                I_Isat[j][Iter] = I_Isat_i[j]
                VP[j][Iter] = VPL2_i[j]
                ET[j][Iter] = ET_i[0][j]
                As[j][Iter] = Ap
            ...
            if Iter > 0:
                ...
                # First we need to calculate the Child-Langmuir sheath corrections for each measurement after the first iteration since the first iteration assumes the area term is the same in all measurements. I.e. the probe area.
                # The Debye lengths differ for each measurement and the sheath area is a function of this.
                
                
                # Calculating A and B parameters
               
                A = S.Params(RpL[j][Iter-1])[0] 
                B = S.Params(RpL[j][Iter-1])[1] 
                
                # Calculating ion corrections for each file 
            
                # Calculate the difference between plasma potential and bias voltage using list comprehension
                
                differences = VP[j][Iter-1] - V[j]
                
                # Ion current corrections
               
                Icor = S.IonCurrCor(RpL[j][Iter-1], e, NE[j][Iter-1], As[j][Iter-1], M_Ar, ET[j][Iter-1], differences, A, B)[0]
                
                # Creating array of bias voltages associated for each of the corrected ion current values
                
                Vcor = -(S.IonCurrCor(RpL[j][Iter-1], e, NE[j][Iter-1], As[j][Iter-1], M_Ar, ET[j][Iter-1], differences, A, B)[1]-VP[j][Iter-1])
                
                # Next we need to take these corrected ion currents and create linear fits to isolate electron current and perform the rest of the analysis, until 
                # convergence is reached for updated ion current
                
                # Step 1: Electron Current Calculation
                
                # Step 1.a: Calculate best fit line for ion saturation current
                
                model = np.polyfit(np.array(Vcor).flatten(),np.array(Icor).flatten(),1)      # Returns the slope, "model[0]" and intercept, "model[1]" for the line of best fit in the region
                predict = np.poly1d(model)
                
                # Step 1.b: Calculate the ion saturation currents for all bias voltage values
                Isat_cor = [predict(x) for idx, x in enumerate(V)]
                
                # Step 1.c: Calculate the updated electron current 
                
                Ie_cor = [x-Isat_cor[idx] for idx, x in enumerate(I)]
                
                # Step 2: Find the plasma potential
                # Plasma potential can be calculated using one of the original functions which takes electron current as input
                
                # Starting with Lobbia's method for calculating plasma potential
                V_Plasma_cor = PlasmaPot_v2(F, Ie_cor, V)
                
                # And this is the new method used in this code that has thus far proven to be superior. This is used in subsequent calculations.
                V_Plasma_cor_2 = PlasmaPot(F, I_ZeroX, Ie_cor, V)
                
                # Step 3: Electron temperature calculation
                
                ElT = ElectronTemp(F,Ie_cor,Vfloat,V_Plasma_cor_2,V,M_Ar,m)[0]
                
                ET[j][Iter] = ElT[j]
                
                # Step 4: Plasma properties calculation
                
                PProps_i = PlasmaProps(Shape, e, As[j][Iter-1], d/2, Vfloat, V_Plasma_cor_2, Ie_cor, M_Ar, m, ElT, I_ZeroX, F, I, V)
                
                # Calculating values for next iteration.
                
                RpL[j][Iter] = PProps_i[3][j]
                NE[j][Iter] = PProps_i[0][j]
                NI[j][Iter] = PProps_i[1][j]
                I_Isat[j][Iter] = PProps_i[2][j]
                VP[j][Iter] = V_Plasma_cor_2[j]
                CV[j][Iter] = np.mean(Icor[j])
                As[j][Iter] = CL_Sheath(d/2,Ap,RpL[j][Iter-1],'cylindrical')
              
                Iter += 1


# Checking if solution has converged for each file
...
if all(ele >= 3 for ele in count):
    print("All solutions successfully converged")
    
    # Provide the location for where you want the excel file to be created
    Loc = 'C:/Users/nnuzz_rpjwflj/OneDrive/Documents/Asif-Nick Business & Thruster Stuff/2025 LP Data Analysis/5-13-25 Results'

    # The following block of code is to remove the entire filepath from the files leaving only the file names which is then passed to the 
    # WriteExcel() function
    ...
    for i in range(len(F)):
        path = F[i]
        PATH.append(os.path.basename(path).split('/')[-1])
    
    # Next we need to populate some variables for the write to excel function that are the final calculated results of the final iteration values    
   
    VPexc = np.zeros(len(F))
    NEexc = np.zeros(len(F))
    I_Isatexc = np.zeros(len(F))
    ETexc = np.zeros(len(F))
    
    VPexc = [VP[ele][NIter-1] for ele, val in enumerate(VPL2_i)]
    NEexc = [NE[ele][NIter-1] for ele, val in enumerate(NE_i)]
    I_Isatexc = [I_Isat[ele][NIter-1] for ele, val in enumerate(I_Isat_i)]
    ETexc = [ET[ele][NIter-1] for ele, val in enumerate(ET_i)]
    
# Writing final results to excel file
WriteExcel(PATH, Loc, Vfloat, VPL2_i, I_Isat_i, ET_i, NE_i)
    

#else:
#    print("Solution(s) did NOT converge. Try increasing number of iterations or relaxing convergence criteria if possible")
'''
Loc = 'C:/Users/nnuzz_rpjwflj/OneDrive/Documents/Asif-Nick Business & Thruster Stuff/2025 LP Data Analysis/52925/500th Recycle'
PATH = []
for i in range(len(F)):
    path = F[i]
    PATH.append(os.path.basename(path).split('/')[-1])
...
# Writing final results to excel file
WriteExcel(PATH, Loc, Vfloat, VPL, I_Isat_i, ET_i[0], NE_i)

# Plotting the solution iterations for visual verification of convergence
'''
plt.figure()
for i in range(len(CV)):
    plt.plot(range(0,NIter),CV[i])  
    
    tick_locations = range(0,NIter)
    plt.xticks(tick_locations)
    
    plt.title("Convergence Plot")
    plt.xlabel("Iteration Number")
    plt.ylabel("Mean of Corrected Ion Current")
    plt.grid(color='k', linestyle='-', linewidth=0.5)          
'''
